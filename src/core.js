...
if (typeof window.tastyPlugShutDown != 'undefined')
    window.tastyPlugShutDown();
(function ($) {
    var afktime = Date.now(), drag = false, hidevideo = false, joincd = false,
        version = '<%= version %>', commands = {}, tos = {}, boothcd = false,
        hover = false, room = location.pathname, lastchat, curvotes = {},
        emotes = {},
        settings = {
            show: true,
            autowoot: false,
        ...
        tastyplugUIStart = '\
            <div id=tp-room></div>\
            <div id=tastyplug-ui style="position: absolute">',
        tastyplugUIMid = '<div id=tp-title>TastyPlug <img class=tastyplug-icon src=https://tastyplug.tastycat.org/images/tastybot.png></div>',
        tastyplugUIEnd = '</div>';


    function AddUIButton(id, text, enabled, secClass, hasDragHandle) {
        tastyplugUIMid += '<div class="' + ((typeof secClass === "undefined") ? 'tp-mainbutton tp-toggle' : 'tp-secbutton tp-' + secClass)
            + (enabled ? ' button-on' : '') + '" id="tp-' + id + '">';

        if (typeof hasDragHandle !== "undefined") {
            tastyplugUIMid += '<div class="icon icon-drag-handle"></div>';
        }

        tastyplugUIMid += '<span>' + text + '</span></div>';
    }
    ...
    function startup() {
        ...
        if (settings.autojoin) {
            afkCheck();
            if (!getLocked() && API.getWaitListPosition() == -1 && API.getDJ() && API.getDJ().id != API.getUser().id)
                join();
        }
        legacyChat.toggle(settings.legacychat);
        tastyMojis.toggle();
        Chat('init', 'TastyPlug v' + version + ' <%= version_title %>');
        console.log('[TastyPlug v' + version + '] <%= version_title %>');
        resize();
    }
    ;
        ...
        if (a) {
            for (var i in settings) {
                if (typeof a[i] != 'undefined') {
                    if (a[i] !== null && a[i].isArray && settings[i] !== null && settings[i].isArray)
                        settings[i] = a[i];
    ...
    function loadUI() {
        ...
        tastyplugUIMid += '\
        	<a href=https://emotes.tastycat.org target=_blank>\
        		<div class=tp-mainbutton id=tp-listemotes><span>Emotes List</span></div>\
        	</a>';

        $('body').append(tastyplugUIStart + tastyplugUIMid + tastyplugUIEnd);
        if (room == '/tastycat')
            $('#waitlist-button').append('<span class="eta"></span>');
    ...
    function loadEvents() {
        var apiEventHandlers = {};
        apiEventHandlers[API.CHAT] = eventChat;
        apiEventHandlers[API.USER_JOIN] = eventJoin;
        apiEventHandlers[API.WAIT_LIST_UPDATE] = eventWLUpd;
        apiEventHandlers[API.ADVANCE] = eventDjAdv;
        apiEventHandlers[API.CHAT_COMMAND] = eventCommand;
        apiEventHandlers[API.VOTE_UPDATE] = refreshMehs;
        apiEventHandlers[API.VOTE_UPDATE] = eventVote;
        API.on(apiEventHandlers);
        ...
        if (typeof tplugUI.draggable != "undefined")
            tplugUI.draggable(dragopts);
        ...
        $('#tp-addmention').click(function () {
            ...
            if (!a)
                return Chat('error', 'Please enter at least one word!');
            for (var i = 0; i < a.length; i++) {
                a[i] = a[i].trim().toLowerCase();
                if (a[i].length < 3)
                    Chat('error', 'Did not add: ' + _.escape(a[i]) + ' (too short)');
                else if (settings.msgs.indexOf(a[i]) > -1)
                    Chat('error', 'Did not add: ' + _.escape(a[i]) + ' (already on list)');
                else
                    settings.msgs.push(a[i]);
            }
        ...
        $('#tp-delmention').click(function () {
            ...
            if (settings.msgs.indexOf(a) > -1) {
                ...
                Chat('error', 'That word isn\'t in the mentions list!');
        });
        $('#tp-listmention').click(function () {
            ...
            for (var i = 0; i < a.length; i++)
                b.push(_.escape(a[i]));
            if (a.length)
                return Chat('info', 'Chat mentions list:<br>' + b.join('<br>'));
    ...
    function loadEmotes() {
        ...
        fetchJSON(links.twitchSub, function (err, data) {
            if (err)
                Chat('error', 'Could not load the Twitch Subscriber emotes. Refresh and/or try again later.');
            else {
            ...
            fetchJSON(links.twitch, function (err, data) {
                if (err)
                    Chat('error', 'Could not load the Twitch Global emotes. Refresh and/or try again later.');
                else
                ...
                fetchJSON(links.rcs, function (err, data) {
                    ...
                    fetchJSON(links.tp, function (err, data) {
                        if (err)
                            Chat('error', 'Could not load the tastyPlug emotes. Refresh and/or try again later.');
                        else
                            for (var i in data.emotes)
    ...
    function eventChat(a) {
        ...
        if (settings.chatimgs && a.message.toLowerCase().indexOf('nsfw') == -1) {
            ...
            for (var i = 0; i < txts.length; i++) {
                if (/^https?:\/\//i.test(txts[i])) {
                    if (/.(gif|png|jpe?g)/i.test(txts[i]))
                        return checkImg(txts[i], txt);
                    else if (/.(webm|gifv)$/i.test(txts[i]))
                        return checkVid(txts[i], txt);
                }
            }
        }
        ...
        if (!settings.chatmentions || a.uid == API.getUser().id || a.type == 'mention')
            return;
        b = message.toLowerCase().split(' ');
        for (var i = 0; i < settings.msgs.length; i++) {
            if (b.indexOf(settings.msgs[i]) > -1)
                return chatSound();
        }
    }
    ;
    ...
    function eventJoin(a) {
        ...
        if (!a.username)
            return;
        if (!settings.joinnotifs.ranks && !settings.joinnotifs.friends && !settings.joinnotifs.lvl1)
            return;
        ...
        else if (settings.joinnotifs.friends && a.friend)
            b = 'friend';
        else if (settings.joinnotifs.lvl1 && getRank(API.getUser()) >= 2 && a.level == 1)
            b = 'lvl1';
        if (b) {
            if (b == 'lvl1')
                str += '[Lvl 1 - ID: ' + a.id + '] ';
    ...
    function eventDjAdv(a) {
        if (settings.autojoin && !getLocked() && API.getWaitListPosition() == -1)
            join();
        if (settings.autowoot)
            setTimeout(woot, (Math.floor(Math.random() * 10) + 1) * 1000);
        if (!a.dj)
            return;
        if (a.dj.id == API.getUser().id)
            boothcd = false;
    ...
    commands.ban = function (a) {
        ...
        if (!user)
            return Chat('error', 'User not found.');
        if (getRank(user))
            return Chat('error', 'You shouldn\'t ban those with ranks!');
    ...
    commands.kick = function (a) {
        ...
        if (!user)
            return Chat('error', 'User not found.');
        if (user.role || user.gRole)
            return Chat('error', 'You shouldn\'t kick those with ranks!');
    ...
    commands.mute = function (a) {
        ...
        if (!user)
            return Chat('error', 'User not found.');
        if (getRank(user))
            return Chat('error', 'You can\'t mute those with ranks!');
        API.moderateMuteUser(user.id, 1, c);
    };

    commands.commands = function () {
        if (room == '/tastycat')
            Chat('info', 'Tastybot commands: <a href="http://tastycat.org/tastybot/" target="_blank">Click Here</a>');
    ...
    commands.link = function () {
        var b = API.getMedia();
        if (b.format == '1')
            Chat('info', 'Current song: <a href="http://youtu.be/' + b.cid + '" target="_blank">Click Here</a>');
        else
            SC.get('/tracks/' + b.cid, function (c) {
    ...
    commands.pic = function () {
        var b = API.getMedia();
        if (b.format == 1)
            Chat('info', 'Video image: <a href="http://i1.ytimg.com/vi/' + b.cid + '/maxresdefault.jpg" target="_blank">Click Here</a>');
        else
            SC.get('/tracks/' + b.cid, function (c) {
    ...
    function resize() {
        ...
        if (uiwidth + uipos.left > rwidth)
            ui.css('left', rwidth - uiwidth);
        if (uiheight + uipos.top > rheight)
            ui.css('top', rheight - uiheight);
        settings.uipos = ui.position();
    ...
    function urlFix(a) {
        ...
        for (var i = 0; i < a.length; i++)
            if (!a[i].indexOf('http'))
                a[i] = '<a href="' + a[i] + '" target="_blank">' + a[i] + '</a>';
        return a.join(' ');
    }

    function afkCheck() {
        if (settings.autojoin)
            tos.afkInt = setInterval(function () {
    ...
    function custEmotes(txt, message) {
        var keys = Object.keys(emotes), html = txt.html();
        if (!keys.length || typeof html != 'string')
            return;
